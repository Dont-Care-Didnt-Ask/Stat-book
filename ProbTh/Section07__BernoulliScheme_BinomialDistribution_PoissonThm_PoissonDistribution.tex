...
Со схемой Бернулли можно связать последовательность случайных величин 
...
%\begin{defn}
%    \textit{Схема Бернулли}~--- последовательность независимых испытаний, в каждом из которых возможны лишь два исхода~--- <<успех>> и <<неудача>>, при этом успех в каждом испытании происходит с одной и той же вероятностью $p \in (0;1)$, а неудача~--- с вероятностью $q = 1 - p.$
%\end{defn}
...
\begin{defn}
    ...
    %то есть принимает значение $1$ (успех) с вероятностью $p$ и $0$ (неудача) с вероятностью $q = 1 - p$. 
...
\subsubsection{Числовые характеристики $\mathbf{Bi}(n, p)$}
\begin{enumerate}
    \item Математическое ожидание:
    \begin{equation*}
